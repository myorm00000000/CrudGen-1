...
$collection->add('{{ route_name_prefix }}_publish', new Route(
        '/{id}/publish',
        array('_controller' => '{{ bundle }}:{{ entity }}:publish')
));
{% endif %}

{% if 'rank' in field_names %}
$collection->add('{{ route_name_prefix }}_changeRank', new Route(
        '/changerank',
        array('_controller' => '{{ bundle }}:{{ entity }}:changeRank'),
        array('_method' => 'post')
));
{% endif %}
